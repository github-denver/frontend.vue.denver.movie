  ...
  <div class="area_member">
    <Cell>
      <template v-slot:children>
        <Hgroup :attributes="{ heading: { tagName: 'h2', subject: '내 정보', size: 'large' } }">
          <template v-slot:description>
            <p class="description_common">수정할 정보를 입력해 주세요!</p>
          </template>
        </Hgroup>

        <form method="post" enctype="multipart/form-data" @submit.prevent="handleProfile">
          <fieldset>
            <legend class="accessibility">회원정보 수정</legend>
            ...
            <Field :attributes="{ type: 'text', id: 'id', title: '아이디', value: member.id }" @onInsert="handleInsert" />
...
export default {
  ...
  created() {
    Object.entries(this.user).forEach(([key, value]) => {
      if (key in this.member && key !== 'picture') this.member[key] = value;
    });
  },
  computed: {
    ...mapState(['user']),
  ...
  methods: {
    ...mapActions(['checkId', 'checkName', 'checkEmail', 'profile']),
